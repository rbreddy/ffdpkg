...
1. [x] Separate files: main.go, fetch.go, verify.go, install.go.
2. Profiles/data: do not store user data in /opt; use $HOME/.mozilla/firefox.
3. Cache dir: ensure userCacheDir() creates it and use consistently.
4. Atomic install: create temp dir as sibling of installDir, not inside it.
